...
<body class="font-body text-gray-800 antialiased bg-white">
    ...
    @if($globalContact && $globalContact->whatsapp)
        <a href="https://wa.me/{{ $globalContact->whatsapp }}?text={{ urlencode('Halo Sekolah Ekspor Nasional, saya ingin bertanya mengenai program sertifikasi/pelatihan.') }}"
           ...
           class="fixed bottom-6 right-6 z-50 flex items-center justify-center w-14 h-14 rounded-full bg-[#25D366] text-white shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 group active:scale-95"
           aria-label="Hubungi kami di WhatsApp">
            {{-- Pulse rings --}}
            <span class="absolute inset-0 rounded-full bg-[#25D366] opacity-40 animate-ping"></span>
            <span class="absolute inset-0 rounded-full bg-[#25D366] opacity-20 group-hover:scale-125 transition-transform duration-500"></span>
