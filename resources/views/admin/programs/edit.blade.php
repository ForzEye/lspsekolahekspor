...
<div class="max-w-2xl mx-auto">
    ...
    <div class="flex items-center gap-3 mb-6">
        ...
        <h1 class="font-display text-2xl font-bold text-gray-900">Edit Program</h1>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
        <form action="{{ route('admin.programs.update', $program) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="space-y-5">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" id="icon" name="icon" value="{{ old('icon', $program->icon) }}"
                               maxlength="10"
                               class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-primary/30 focus:border-primary outline-none text-2xl transition-colors">
                    </div>
                    <div>
                        ...
                        <select id="mode" name="mode"
                            ...
                            @foreach(['Online', 'Offline', 'Hybrid'] as $m)
                                <option value="{{ $m }}" {{ old('mode', $program->mode) === $m ? 'selected' : '' }}>{{ $m }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="4" required
                              class="w-full border {{ $errors->has('description') ? 'border-red-400' : 'border-gray-200' }} rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-primary/30 focus:border-primary outline-none resize-none transition-colors">{{ old('description', $program->description) }}</textarea>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" id="duration" name="duration" value="{{ old('duration', $program->duration) }}"
                    ...
                    <div>
                        ...
                        <input type="number" id="sort_order" name="sort_order" value="{{ old('sort_order', $program->sort_order) }}" min="0"
                               class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-primary/30 focus:border-primary outline-none transition-colors">
                    </div>
                </div>
                ...
                <div class="bg-soft rounded-xl p-4">
                    <p class="text-xs font-semibold text-gray-500 mb-2">Gambar Icon Saat Ini</p>
                    <img src="{{ $program->icon_image_url }}" alt="{{ $program->title }}" class="h-16 w-16 object-contain rounded-lg">
                </div>
                @endif

                <div>
                    <label for="icon_image" class="block text-sm font-medium text-gray-700 mb-1.5">
                        {{ $program->icon_image ? 'Ganti Gambar Icon' : 'Gambar Icon (opsional)' }}
                    </label>
                    ...
                    <p class="text-gray-400 text-xs mt-1">Rekomendasi ukuran: <strong>200 x 200 piksel (Rasio 1:1)</strong>. Max 2MB. Kosongkan jika tidak ingin mengganti.</p>
                </div>
            ...
            <div class="flex items-center gap-3 mt-8 pt-6 border-t border-gray-100">
                <button type="submit" id="btn-update-program"
