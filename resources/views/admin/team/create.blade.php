...
<div class="max-w-2xl mx-auto">
    <div class="flex items-center gap-3 mb-6">
        ...
        <h1 class="font-display text-2xl font-bold text-gray-900">Tambah Anggota Tim</h1>
    </div>
        ...
        <form action="{{ route('admin.team.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="space-y-5">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" required placeholder="Contoh: Arifin Jaya"
                ...
                <div>
                    ...
                    <select name="position" required class="w-full border border-gray-200 bg-white rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-primary/30 outline-none">
                        ...
                        <option value="Direktur Utama" {{ old('position') === 'Direktur Utama' ? 'selected' : '' }}>Dewan Pengarah (Direktur Utama)</option>
                        <option value="Direktur LSP" {{ old('position') === 'Direktur LSP' ? 'selected' : '' }}>Direktur LSP</option>
                        <option value="Komite Skema" {{ old('position') === 'Komite Skema' ? 'selected' : '' }}>Komite Skema</option>
                        <option value="Manajer Sertifikasi" {{ old('position') === 'Manajer Sertifikasi' ? 'selected' : '' }}>Manajer Sertifikasi</option>
                        <option value="Manajer Mutu" {{ old('position') === 'Manajer Mutu' ? 'selected' : '' }}>Manajer Mutu</option>
                        <option value="Manajer Administrasi" {{ old('position') === 'Manajer Administrasi' ? 'selected' : '' }}>Manajer Administrasi</option>
                    </select>
                </div>
            ...
            <div class="flex items-center gap-3 mt-8 pt-6 border-t border-gray-100">
                <button type="submit" class="bg-accent hover:bg-accent-dark text-white font-bold px-8 py-3.5 rounded-xl text-sm transition-all duration-200 shadow-md">Simpan Anggota</button>
            </div>
        </form>
    </div>
</div>
