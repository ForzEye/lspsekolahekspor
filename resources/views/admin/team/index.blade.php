...
<div class="max-w-7xl mx-auto">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="font-display text-2xl font-bold text-gray-900">Pengurus & Tim</h1>
            <p class="text-gray-500 text-sm mt-0.5">Kelola data struktur organisasi Sekolah Ekspor Nasional.</p>
        </div>
        ...
        <table class="w-full">
            <thead class="bg-soft border-b border-gray-100">
                <tr>
                    ...
                    <th class="text-left px-4 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Jabatan</th>
            ...
            <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-soft transition-colors duration-150 group">
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                ...
                                @else
                                    <div class="w-10 h-10 rounded-full bg-primary flex items-center justify-center text-white font-bold text-xs">{{ $member->initials }}</div>
                                @endif
                                <p class="font-display font-semibold text-gray-900 text-sm">{{ $member->name }}</p>
                            </div>
                        </td>
                        ...
                        <td class="px-4 py-4">
                            <span class="inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full
                                ...
                                <span class="w-1.5 h-1.5 rounded-full {{ $member->is_active ? 'bg-green-500' : 'bg-gray-400' }}"></span>
                                {{ $member->is_active ? 'Aktif' : 'Nonaktif' }}
                            </span>
                        </td>
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('admin.team.edit', $member) }}" class="text-primary hover:text-primary-mid text-xs font-bold border border-primary/20 px-3 py-1.5 rounded-lg transition-colors">Edit</a>
                                <form action="{{ route('admin.team.destroy', $member) }}" method="POST" onsubmit="return confirm('Hapus anggota ini?')">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="4" class="px-6 py-16 text-center text-gray-400">Belum ada data anggota tim.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
