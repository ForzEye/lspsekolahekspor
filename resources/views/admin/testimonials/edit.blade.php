...
<div class="max-w-2xl mx-auto">
    <div class="flex items-center gap-3 mb-6">
        ...
        <h1 class="font-display text-2xl font-bold text-gray-900">Edit Testimonial</h1>
    </div>
            ...
            <div class="space-y-5">
                ...
                <div>
                    ...
                    <select name="rating" required class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-primary/30 outline-none bg-white">
                        @for($i=5; $i>=1; $i--)
                            <option value="{{ $i }}" {{ old('rating', $testimonial->rating) == $i ? 'selected' : '' }}>{{ str_repeat('⭐', $i) }} ({{ $i }} Bintang)</option>
                        @endfor
                    </select>
                </div>
                ...
                <div>
                    ...
                    <textarea name="content" rows="4" required
                              class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-primary/30 outline-none resize-none">{{ old('content', $testimonial->content) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    @if($testimonial->photo)
                        <div class="flex items-center gap-4 mb-3 p-3 bg-soft rounded-xl">
                            <img src="{{ $testimonial->photo_url }}" alt="{{ $testimonial->name }}" class="w-16 h-16 rounded-lg object-cover">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div class="flex items-center">
                        <label class="flex items-center gap-3 cursor-pointer mt-6">
                            <input type="checkbox" name="is_active" value="1" {{ $testimonial->is_active ? 'checked' : '' }} class="w-5 h-5 rounded accent-primary">
