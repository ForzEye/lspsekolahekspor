...
@section('title', 'Edit Langkah Alur')
...
<div class="max-w-2xl mx-auto">
    <div class="flex items-center gap-3 mb-6">
        ...
        <h1 class="font-display text-2xl font-bold text-gray-900">Edit Langkah</h1>
    </div>

    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
        <form action="{{ route('admin.sertifikasi.alurs.update', $alur) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-5">
                <div class="grid sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="type" required class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-primary/30 outline-none bg-white">
                            <option value="tatap_muka" {{ $alur->type == 'tatap_muka' ? 'selected' : '' }}>Tatap Muka</option>
                            <option value="jarak_jauh" {{ $alur->type == 'jarak_jauh' ? 'selected' : '' }}>Jarak Jauh</option>
                        </select>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1.5">Nomor Langkah <span class="text-red-500">*</span></label>
                        <input type="number" name="step_number" value="{{ old('step_number', $alur->step_number) }}" required
                ...
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title', $alur->title) }}" required
                           class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-primary/30 outline-none">
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Deskripsi</label>
                    <textarea name="description" rows="3" class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-primary/30 outline-none resize-none">{{ old('description', $alur->description) }}</textarea>
                </div>

                <div class="grid sm:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1.5">Icon (Emoji)</label>
                        <input type="text" name="icon" value="{{ old('icon', $alur->icon) }}" maxlength="10"
                ...
                <div class="bg-blue-50 p-5 rounded-2xl border border-blue-100">
                    ...
                    <div class="space-y-3">
                        <div>
                            ...
                            <input type="text" name="extra_label" value="{{ old('extra_label', $alur->extra_label) }}"
                        ...
                        <div>
                            ...
                            <textarea name="extra_content" rows="2" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-xs outline-none resize-none">{{ old('extra_content', $alur->extra_content) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-center gap-3 mt-8 pt-6 border-t border-gray-100">
                ...
                <form action="{{ route('admin.sertifikasi.alurs.destroy', $alur) }}" method="POST" onsubmit="return confirm('Hapus langkah ini?')">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="text-red-500 text-sm font-bold px-4 py-3.5 rounded-xl border border-red-100 hover:bg-red-50 transition-colors">Hapus</button>
                </form>
            </div>
        </form>
    </div>
