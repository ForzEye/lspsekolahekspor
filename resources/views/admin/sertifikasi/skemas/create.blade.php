...
<div class="max-w-2xl mx-auto">
    <div class="flex items-center gap-3 mb-6">
        ...
        <h1 class="font-display text-2xl font-bold text-gray-900">Tambah Skema</h1>
    </div>

    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
        <form action="{{ route('admin.sertifikasi.skemas.store') }}" method="POST" enctype="multipart/form-data">
            @csrf

            <div class="space-y-5">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Kode Skema <span class="text-red-500">*</span></label>
                    <input type="text" name="kode" value="{{ old('kode') }}" required placeholder="Contoh: SKM-01"
                           class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-primary/30 outline-none">
                    <input type="hidden" name="level" value="Muda">
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Nama Skema <span class="text-red-500">*</span></label>
                    <input type="text" name="nama" value="{{ old('nama') }}" required placeholder="Contoh: Skema Sertifikasi Ekspor Muda"
                           class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-primary/30 outline-none">
                </div>
                ...
                <div class="grid sm:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="masa_berlaku" value="{{ old('masa_berlaku') }}" placeholder="Contoh: 3 Tahun"
                               class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-primary/30 outline-none">
                    </div>
                    <div>
                ...
                <div x-data="{ units: {{ json_encode(old('units') ?? [['kode' => '', 'nama' => '']]) }} }">
                ...
                <div x-data="{ items: {{ json_encode(old('requirements') ?? ['']) }} }">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="sort_order" value="{{ old('sort_order', 0) }}" class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-primary/30 outline-none">
                    </div>
            ...
            <div class="flex items-center gap-3 mt-8 pt-6 border-t border-gray-100">
                <button type="submit" class="bg-accent hover:bg-accent-dark text-white font-bold px-8 py-3.5 rounded-xl text-sm transition-all duration-200 shadow-md">Simpan Skema</button>
            </div>
        </form>
    </div>
</div>
